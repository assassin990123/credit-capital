...
describe("Rewards Vault", function () {
  it("Rewards end to end", async function () {
    ...
    expect(await rewards.connect(user).claim(lp.address, user.address)).to.emit(rewards, "Claim").withArgs(lp.address, user.address, 56)
    expect(await rewards.connect(user).claim(lp.address, user2.address)).to.emit(rewards, "Claim").withArgs(lp.address, user2.address, 16)
