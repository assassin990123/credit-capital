...
describe("Credit Capital Platform Token", function () {
	...
	it("Should be mintable", async function () {
		...
		try {
			...
			const revert = new RegExp("AccessControl: account " + nonMinter.address.toLowerCase() + " is missing role " + minterRole);
	...
	it("Should be burnable", async function () {
		...
		// check the balance
		expect(await capl.balanceOf(deployer.address)).to.equal(mintAmount - burnAmount);
		...
		expect(await capl.balanceOf(deployer.address)).to.equal(mintAmount - (burnAmount * 2));
